...
public class AddCartAction extends ActionSupport implements SessionAware {
	...
	public String execute() {
		...
		//sessionにloginIdが格納されている、かつtempUserIdが格納されていないとき
		//sessionにtempUserIdを格納する
		if(!(session.containsKey("loginId")) && !(session.containsKey("tempUserId"))) {
			CommonUtility commonUtility = new CommonUtility();
			session.put("tempUserId", commonUtility.getRandomValue());
		}

		//sessionにloginIdが格納されているとき
		//userIdにsessionのloginIdを代入する
		if(session.containsKey("loginId")){
			userId = String.valueOf(session.get("loginId"));
		}

		//sessionにloginIdが格納されていない、かつtempUserIdが格納されているとき
		//userIdとtempUserIdにsessionのtempUserIdを格納する。
		if(!(session.containsKey("loginId")) && session.containsKey("tempUserId")) {
			userId = String.valueOf(session.get("tempUserId"));
			tempUserId = String.valueOf(session.get("tempUserId"));
		}

		//productCountに文字列を格納する
