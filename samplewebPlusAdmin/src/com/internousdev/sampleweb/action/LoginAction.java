...
public class LoginAction extends ActionSupport implements SessionAware{
	...
	public String execute() {
		...
		//savedLoginIdがtrueならば、sessionにtrueと追加し、logiIdも追加する。
		//falseならば、sessionにfalseと追加し、loginIdは削除する。
		...
		//loginIdErrorMessageList/passwordErrorMessageListの格納個数がともに0でなければ、
		//sessionに追加する。そして、loginedには0を入れる。
		...
		//sessionにmCategoryListがない場合、追加する。
		...
		//管理者（statusが1）であれば管理者としてログイン
		if(userInfoDTO.getStatus()!=null){
			if(userInfoDTO.getStatus().equals("1")){
				result = "master";
			}else{
				//loginIdとpasswordが存在する、かつloginedを1に更新したならば、
				//sessionのloginIdにuserIdを入れる
				if(userInfoDao.isExistsUserInfo(loginId, password)){
					if(userInfoDao.login(loginId,password)>0){
						...
						//String.valueOf() ⇒カッコ内の要素をString型に変換する
						//カートのloginIdを更新。そして更新数が1以上であれば、
						//loginIdよりユーザーデータを抽出し、sessionに格納。
						count = cartInfoDao.linkToLoginId(String.valueOf(session.get("tempUserId")),loginId);

						//cartの更新が1個以上であれば、settlement.jspに遷移
						if(count > 0){
						...
						//テーブルカートの更新がなければSUCCESS
						else {
							result = SUCCESS;
						}
					}
					//sessionのloginedに1を入れる
