...
public class DeleteCartAction extends ActionSupport implements SessionAware{
	...
	public String execute(){
		...
		//checkListがnullのときメッセージを追加して、sessionに格納する
		if(checkList==null){
		...
		//cart削除が行われなかったら、メッセージを追加して、sessionに格納する
		//cart削除が行われたら、userIdをnullにして、sessionにloginIdがある場合はuserIdに代入
		//cart削除が行われて、sessionにtempUserIdがある場合はuserIdに代入
		if(count <= 0){
			checkListErrorMessageList.add("チェックされていません。");
			session.put("checkListErrorMessageList", checkListErrorMessageList);
			return ERROR;
